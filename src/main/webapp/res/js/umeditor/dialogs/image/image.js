...
//                }
    ...
    var Upload = {
        ...
        uploadTpl: '<div class="edui-image-upload%%">' +
            ...
            '<input style=\"filter: alpha(opacity=0);\" class="edui-image-file" type="file" hidefocus name="upfile[]" accept="image/gif,image/jpeg,image/png,image/jpg,image/bmp" multiple/>' +
            '</form>' +
            '</div>',
        ...
        getToken: function(){
        	// 获取上传的token
            $.get(this.editor.options.tokenUrl, function(res){
            	Upload.token = res.obj;
            });
        },
        ...
        uploadComplete: function(r){
            ...
            try{
                var json = eval('('+r+')');
                var state = json.key ? "SUCCESS" : "上传失败";
                
                // 获取图片真实的地址
                $.post(me.editor.options.fileUrl, {"key": json.key}, function(res){
                    Base.callback(me.editor, me.dialog, res.obj, state);
                });
            }catch (e){
